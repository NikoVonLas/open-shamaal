...
if ((($fid == $id) && ($owner_id == $player_id) && ($owner_city == 0))|| (($owner_id == $player_clan) && ($owner_city == 3) && ($allow == 1)))
{
	if ($act == "set")
	{
		...
		$num = 0;
		$name= "";
		$SQL="select sw_stuff.name,sw_stuff.pic,sw_stuff.price,sw_obj.max_cond,sw_obj.cur_cond,sw_obj.num from sw_stuff inner join sw_obj on sw_stuff.id=sw_obj.obj where sw_obj.owner=$player_id and room=0 and sw_obj.id=$obj_id and sw_stuff.client=0";
		$row_num=SQL_query_num($SQL);
		while ($row_num){
			$name=$row_num[0];
			$pic=$row_num[1];
		...
		if ($num >= $count)
		{
			if (($count > 0) && ($setprice >= 0))
			{
				if ($name <> "")
				{
					...
					$shp = "<table width=100% height=100%><tr><Td align=center><table><tr><td colspan=2 align=center><b>$name</b></td></tr><tr><td width=64 align=center><img src=/assets/img/stuff/$pic></td><td><table><tr><td>Количество: </td><td><font color=007700>$count</font></td></tr><tr><td>Состояние: </td><td><font color=007700>$cur_cond / $max_cond</font></td></tr><tr><td>Оценка предмета: </td><td><font color=888800><b>$setprice злт.</b></font></td></tr></table></td></tr></table></td></tr></table>";
