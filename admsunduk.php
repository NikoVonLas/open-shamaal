...
if ( $fid != "" && ( $owner_id == $player_id && $owner_city == 0 || $owner_id == $player_clan && $owner_city == 3 ) )
{
    ...
    if ( $act == "set" )
    {
        ...
        $num = 0;
        $name = "";
        $SQL = "select sw_stuff.name,sw_stuff.pic,sw_stuff.price,sw_obj.max_cond,sw_obj.cur_cond,sw_obj.num,sw_stuff.weight from sw_stuff inner join sw_obj on sw_stuff.id=sw_obj.obj where sw_obj.owner={$player_id} and room=0 and sw_obj.id={$obj_id} and sw_stuff.client=0";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            $name = $row_num[0];
