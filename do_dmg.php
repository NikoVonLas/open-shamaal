...
if ($loop == 0 || $npc_kick == 0) {
    ...
    function dead()
    {
        ...
        if ($pl_npc[$target_id] == 1) {
            $sql_text .= ",online=$cur_time-130,room=60000";
        }
        ...
        if ($pl_city[$target_id] == 9) {
            $sql_text .= ",room=1653";
        } elseif (($pl_map_name[$player_id] <> "") && ($pl_no_pvp[$target_id] <> 2)) {
    ...
    function inIsland($p_id)
    {
        ...
        if (($pl_room[$p_id] >= 1327) && ($pl_room[$p_id] <= 1330)) {
            return true;
        } elseif (($pl_room[$p_id] >= 1315) && ($pl_room[$p_id] <= 1319)) {
            return true;
        } elseif (($pl_room[$p_id] >= 2057) && ($pl_room[$p_id] <= 2156)) {
            return true;
        }
    ...
    function checkerror()
    {
        ...
        /*if ($error == 1) {
            if (($pl_npc[$target_id] == 0) && (($pl_npc[$player_id] == 0)) && $pl_level[$player_id] > 9) {
                if ($game_skill_bad[$skill_id][$num] == 1 && ($skill_id != 0 || $num != 3)) {
                    $error = 0;
...
//          }
            ...
            if ($npc_kick == 0) {
                if (($game_skill_wepon[$skill_id][$num]<>0) && ($game_skill_wepon[$skill_id][$num] <> $wepontype[$player_id])) {
                    if (($game_skill_wepon[$skill_id][$num] <> 6) || ($wepontype[$player_id] <> 0)) {
                        ...
                        if ($game_skill_wepon[$skill_id][$num] == 7 && $wepontype[$player_id] == 24) {
                            $problem = false;
                        }
                        if ($game_skill_wepon[$skill_id][$num] == 4 && $wepontype[$player_id] >= 20 && $wepontype[$player_id] <= 24) {
                            $problem = false;
                        }
            ...
            if (((($pl_city[$player_id] <> 0)||($pl_npc[$player_id]==1)) || (($pl_city[$target_id] <> 0)||($pl_npc[$target_id]==1))) || (($pl_clan[$target_id] == $pl_clan[$player_id]) && ($pl_clan[$player_id] <> 0))) {
                ...
                if (($game_skill_bad[$skill_id][$num] == 1) && (($pl_no_pvp[$player_id] <> 2) || ($pl_no_pvp[$target_id] <> 2)) || ($game_skill_badOnly[$skill_id][$num] == 1)) {
                    ...
                    if ((($pl_bad[$target_id] == 2) && ($pl_npc[$target_id] == 1)) || (($pl_npc[$player_id] == 1) && ($pl_bad[$player_id]>0) && ($pl_npc[$target_id] == 0))) {
                        $color = 3;
                    } elseif (($pl_bad[$target_id] == 1) && ($pl_npc[$target_id] == 1)) {
                        $color = 2;
                    } elseif ($pl_npc[$target_id] == 1) {
                        $color = 1;
                    } else {
                    ...
                    if ($game_skill_badOnly[$skill_id][$num] == 1) {
                        ...
                        if ($pl_npc[$target_id] == 1 || /*$pl_level[$player_id] < 10 ||*/ $skill_id != 0 || $num != 3) {  // сосульками можно бить всех, но не в Академии
                            if ($npc_kick == 0) {
        ...
        if ($error == 1) {
            if ($game_skill_need_obj[$skill_id][$num] > 0 && $pl_npc[$player_id] == 0) {
...
// $cur_time - в секундах
if (($cur_balance <= $cur_time - $balance+1) || ($npc_kick == 1)) {   //+1 наоборот помагает балансу и казалось бы, должен исключить ситуации с Баланс не восстановлен. Может быть проблема на клиенте...
    if ($npc_kick == 0) {
    ...
    if (file_exists(realpath(__DIR__) . "/skills/$skill_id.php")) {
        ...
        if (checkerror()) {
            if ($npc_kick == 0) {
                if ($pl_aff_speed2[$player_id] > $cur_time) {
                    $ref = 4;
                } elseif ($pl_aff_speed[$player_id] > $cur_time) {
                    $ref = -4;
                } else {
                    $ref = 0;
                }
            ...
            if (($pl_block[$target_id] <> $kickto) || ($game_skill_canblock[$skill_id][$num] == 0)) {
                if (checkdex($pl_dex[$player_id], $pl_dex[$target_id], $game_skill_canmiss[$skill_id][$num]) || ($game_skill_canmiss[$skill_id][$num] == 0)) {
                    ...
                    if ($game_skill_do_SQL[$skill_id][$num] <> "") {
                        ...
                        SQL_do($SQL);
                        SQL_do($game_skill_do_SQL[$skill_id][$num]);
                    }
                    ...
                    if ($game_skill_dmgtype[$skill_id][$num] == 1) {
                        if ($dmg < 1) {
                            ...
                            if (($a == 1) && ($pl_npc[$target_id] == 0)) {
                                ...
                                $ob_condition = 0;
                                $ob_name = '';
                                $SQL="select sw_obj.id, sw_obj.cur_cond, sw_stuff.name from sw_obj inner join sw_stuff on sw_obj.obj=sw_stuff.id where owner=$target_id and room=0 and active=1 and sw_stuff.obj_place=$plac order by rand() limit 0,1";
                                $row_num=SQL_query_num($SQL);
                                while ($row_num) {
                                    $ob_id=$row_num[0];
                                    $ob_condition=$row_num[1];
                                    $ob_name=$row_num[2];
                                    $row_num=SQL_next_num();
                                }
                                if ($result) {
                                ...
                                if ($ob_id > 0) {
                                    ...
                                    if ($breaking_text <> "") {
                                        $SQL = "update sw_users SET mytext=CONCAT(mytext,'$breaking_text') where id = $target_id and npc=0";
                                        SQL_do($SQL);
                                    }
                    ...
                    } elseif (($game_skill_dmgtype[$skill_id][$num] <=5)) {
                        if ($dmg < 1) {
                            $c = $game_skill_count[$skill_id][$num]+1;
                    ...
                    } elseif ($game_skill_dmgtype[$skill_id][$num] == 999) {
                        // Квестовые скилы
                        if ($pl_level[$player_id] < 10) {
                            // Academy
                            if ($pl_npc[$target_id] == 0) {
                                // Attacking other player
                                $dmg = -rand(1, 2);
                            } else {
                                // ToDo: Потом сделать логику что у мобов в базе может быть указано, к каким скилам у него слабость... тогда при слабости урон будет усилен в N раз
                                if ($pl_def1[$target_id] == 999 && $pl_def2[$target_id] == 999) {
                                    $dmg = $dmg * 3;
                                }
                            }
                        } else {
                        ...
                        // Если телепатка или шаманка по квестовым мобам - зарезать урон
                        if ($dmg < 0 && $pl_def1[$target_id] == 999 && $pl_def2[$target_id] == 999) {
                            $dmg = -1;
                        }
                    }
                    ...
                    if (($dmgtext <> "") && ($game_skill_dmgtype[$skill_id][$num] <> 3)) {
                        if ($dmg < 0 && $pl_def1[$target_id] != 999 && $pl_def2[$target_id] != 999) { // выключаем допы для очень защищённых (у которых 999 от всего)
                        ...
                        if ($pl_npc[$target_id] == 1) {
                            if ($pl_give[$target_id] == 0) {
                                ...
                                if ($percent > $rnd) {
                                    ...
                                    $SQL="INSERT INTO sw_logs (OWNER, DT, TYPE, GOLD, EXP, TEXT) VALUES ('".$player_id."', NOW(), 'NPCKILL', '$rnd', 0, 'Player killed npc ".$target_id."')";
                                    SQL_do($SQL);
                                ...
                                $SQL="INSERT INTO sw_logs (OWNER, DT, TYPE, GOLD, EXP, TEXT) VALUES ('".$player_id."', NOW(), 'NPCKILL', 0, 0, 'Player killed npc ".$target_id."')";
                                SQL_do($SQL);
                                ...
                                } elseif ($pl_givemore[$target_id] > 0) {
                                    $rnd = rand(1, 10000);
                    ...
                    if (($game_skill_bad[$skill_id][$num] == 1) && ($dead == 0)) {
                        ...
                        } elseif ($pl_madeby[$player_id] > 0 && $pl_npc[$target_id] == 1) {
