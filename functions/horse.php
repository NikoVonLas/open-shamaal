...
if ( $text != "" )
{
    ...
    $p = "";
    $SQL = "select count(*) from sw_pettype where selling = true";
    $row_num = sql_query_num( $SQL );
    while ( $row_num )
    {
        $count = $row_num[0];
    ...
    if ( $action == "buy" )
    {
        ...
        $SQL = "select name,food,str,pic,min_speed,max_speed,id,loyalty,price, selling from sw_pettype where id={$id} and selling = 1";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            $h_name = $row_num[0];
        ...
        if ( $c < 5 )
        {
            if ( $h_price <= $gold )
            {
                $SQL = "insert into sw_pet (name,owner,food,max_food,str,max_str,min_speed,max_speed,loyalty,pic,active, selling) values ('{$h_name}',{$player_id},{$h_food},{$h_food},{$h_str},{$h_str},{$h_min_speed},{$h_max_speed},{$h_loyalty},'{$h_pic}',1, {$h_selling})";
                sql_do( $SQL );
        ...
        {
            print "<script>alert('Количество животных не может превышать 5.');</script>";
        }
    }
    $i = 0;
    for ( ; $i < $count; ++$i )
    ...
    if ( $count != 0 )
    {
        ...
        $SQL = "select name,food,str,pic,min_speed,max_speed,id,loyalty,price from sw_pettype where selling = true  limit {$page},1";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            $h_name = $row_num[0];
