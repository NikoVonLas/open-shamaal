		...
		if ($obj_specif == 0)
		{
			if ($obj_name <> "")
			{
				...
				else 
				{
					...
					// Если у предмета есть требования по скилам (нужно ли проверять, может обьект уже нельзя одеть по статам)
					if ($isCanWear && $obj_skillId > 0)
					{
						$playerSkillLevel = 0;
					...
					// Если статы персонажа удовлетворяют требованиям вещи
					if ($isCanWear)
					{
						// Снимим вещь которая уже одета на персонаже, если она есть
						if ($wobj_id <> "")
						{
							if ($obj_place <> 2 || $p > 1)
							{
				...
				// Если мы что-то меняли то сохраним
							...
							if ($clan == 0)
								{
                                    ...
                                    while ($row_num) {
                                        ...
                                        $r = rand(0,9999999);
                                        $SQL="select * from sw_clan where id = $r";
                                        $row_num=SQL_query_num($SQL);
                                    }
