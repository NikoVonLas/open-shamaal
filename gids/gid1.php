...
if ( 0 < $player_room )
{
    ...
    if ( $build == 1 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat,weight) values ({$max},'Сундук','sunduk','Сундук',0,{$player_id},NOW(),100)";
            sql_do( $SQL );
        ...
        $text = "Дом";
        $t = "";
        $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=/assets/img/stuff/else/gifsklad.gif></td><td><b>Название создаваемого дома:</b> `Дом`<br><br>Выберите направление строительства дома относительно вашей текущей комнаты.</td></tr></table>";
