...
if ( 0 < $player_room )
{
    ...
    if ( $owner_id == $player_clan && $owner_typ == 1 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            ++$max;
            $SQL = "Insert into sw_map (id,name,location,pic,{$mp[$dir]}id,{$mp[$dir]}name,owner_name,owner_id,owner_typ,no_pvp) values ({$max},'Ателье',19,'',{$player_room},'{$map_name}','{$litle}',{$player_clan},1,1)";
            sql_do( $SQL );
            $SQL = "update sw_map set {$mn[$dir]}id = {$max},{$mn[$dir]}name='Ателье' where id={$player_room}";
            sql_do( $SQL );
            $SQL = "delete from sw_obj where id={$obj_id} and owner={$player_id}";
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Ткачество','tkat','Ткачество',3,{$player_clan},NOW())";
            sql_do( $SQL );
            $player['room'] = 0;
        ...
        $text = "Ателье клана";
        $t = "";
        $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=/assets/img/stuff/else/gid.gif></td><td><b>Название создаваемой комнаты:</b> `Ателье`<br><br>Выберите направление строительства ателье относительно вашей текущей комнаты.</td></tr></table>";
        ...
        $t .= "</td><td valign=top><i>Постарайтесь выбрать наиболее подходящее место для ателье, так как у вас не будет возможности её перенести в другое место.<br><br> Ателье смогут пользоваться все члены Вашего клана.</i></td></tr></table></td></tr></table>";
        print "<script>window.top.domir('{$text}','{$t}');</script>";
    }
    else
    {
        print "<script>alert('Ателье может принадлежать только клану.');</script>";
    }
}
?>
