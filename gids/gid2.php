...
if ( 0 < $player_room )
{
    ...
    $mn[7] = "j_";
    $mn[8] = "jv_";
    if ( $owner_id == $player_id && $owner_typ == 0 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            ++$max;
            $SQL = "Insert into sw_map (id,name,location,pic,{$mp[$dir]}id,{$mp[$dir]}name,no_pvp,regen,owner_id,owner_name) values ({$max},'Усыпальница',19,'',{$player_room},'{$map_name}',1,1,{$player_id},'{$player_name}')";
            sql_do( $SQL );
            $SQL = "update sw_map set {$mn[$dir]}id = {$max},{$mn[$dir]}name='Усыпальница' where id={$player_room}";
            sql_do( $SQL );
            $SQL = "delete from sw_obj where id={$obj_id} and owner={$player_id}";
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Прописаться','prey','Сундук',0,{$player_id},NOW())";
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Выписаться','notprey','Сундук',0,{$player_id},NOW())";
            sql_do( $SQL );
            $player['room'] = 0;
        ...
        $text = "Создание усыпальницы игрока";
        $t = "";
        $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=/assets/img/stuff/else/gid.gif></td><td><b>Название создаваемой комнаты:</b> `Усыпальница`<br><br>Выберите направление строительства комнаты относительно вашего текущего местоположения.</td></tr></table>";
        ...
        $t .= "</td><td valign=top><i>Постарайтесь выбрать наиболее подходящее место для усыпальницы, так как у вас не будет возможности её перенести в другое место.<br><br> В этом типе контаты, вы сможете появляться после смерти.</i></td></tr></table></td></tr></table>";
        print "<script>window.top.domir('{$text}','{$t}');</script>";
    }
    else
    {
        print "<script>alert('Усыпальницу можно строить только в доме.');</script>";
    }
}
?>
