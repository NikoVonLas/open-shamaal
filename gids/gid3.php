...
if ( 0 < $player_room )
{
    ...
    if ( $player_clan != 0 )
    {
        if ( $build == 2 )
        {
            if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
            {
                ...
                ++$max;
                $SQL = "Insert into sw_map (id,name,location,pic,{$mp[$dir]}id,{$mp[$dir]}name) values ({$max},'Магазин [{$litle}]',19,'',{$player_room},'{$map_name}')";
                sql_do( $SQL );
                $SQL = "update sw_map set {$mn[$dir]}id = {$max},{$mn[$dir]}name='Магазин [{$litle}]' where id={$player_room}";
                sql_do( $SQL );
                $SQL = "delete from sw_obj where id={$obj_id} and owner={$player_id}";
                sql_do( $SQL );
                $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Магазин','buy','Магазин',3,{$player_clan},NOW())";
                sql_do( $SQL );
                $player['room'] = 0;
            ...
            $text = "Создание магазина клана";
            $t = "";
            $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=/assets/img/stuff/else/gid.gif></td><td><b>Название создаваемого магазина:</b> `Магазин [{$litle}]`<br><br>Выберите направление строительства магазина относительно вашей текущей комнаты.</td></tr></table>";
            ...
            $t .= "</td><td valign=top><i>Постарайтесь выбрать наиболее подходящее место для магазина, так как у вас не будет возможности его перенести в другое место.<br><br> В этом типе магазина, глава клана и казначеи смогут выставлять вещи на продажу.</i></td></tr></table></td></tr></table>";
    ...
    {
        print "<script>alert('Магазин может принадлежать только клану.');</script>";
    }
}
?>
