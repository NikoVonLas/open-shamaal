...
if ( 0 < $player_room )
{
    ...
    if ( $owner_id == $player_clan && $owner_typ == 1 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            ++$max;
            $SQL = "Insert into sw_map (id,name,location,pic,{$mp[$dir]}id,{$mp[$dir]}name,no_pvp,regen,owner_id,owner_typ,owner_name) values ({$max},'Усыпальница',19,'',{$player_room},'{$map_name}',1,1,{$player_clan},1,'{$litle}')";
            sql_do( $SQL );
            ...
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Прописаться','prey','Сундук',3,{$player_id},NOW())";
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat) values ({$max},'Выписаться','notprey','Сундук',3,{$player_id},NOW())";
            sql_do( $SQL );
            $player['room'] = 0;
    ...
    {
        print "<script>alert('Усыпальницу можно строить только в здании клана.');</script>";
    }
}
?>
