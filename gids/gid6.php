...
if ( 0 < $player_room )
{
    ...
    if ( $owner_id == $player_clan && $owner_typ == 1 )
    {
        if ( $mp[$dir] != "" && $do == "build" && $map[$dir] == 0 )
        {
            ...
            ++$max;
            $SQL = "Insert into sw_map (id,name,location,pic,{$mp[$dir]}id,{$mp[$dir]}name,owner_name,owner_id,owner_typ,no_pvp) values ({$max},'Склад [{$litle}]',19,'',{$player_room},'{$map_name}','{$litle}',{$player_clan},1,1)";
            sql_do( $SQL );
            $SQL = "update sw_map set {$mn[$dir]}id = {$max},{$mn[$dir]}name='Склад [{$litle}]' where id={$player_room}";
            sql_do( $SQL );
            $SQL = "delete from sw_obj where id={$obj_id} and owner={$player_id}";
            sql_do( $SQL );
            $SQL = "insert into sw_object (id,name,what,text,owner_city,owner,dat,weight) values ({$max},'Общий склад','sunduk','Сундук',3,{$player_clan},NOW(), 250)";
            sql_do( $SQL );
            $player['room'] = 0;
        ...
        $text = "Создание склада клана";
        $t = "";
        $t .= "<table><tr><td><table><tr><td width=75 align=center><img src=/assets/img/stuff/else/gid.gif></td><td><b>Название создаваемого склада:</b> `Склад [{$litle}]`<br><br>Выберите направление строительства склада относительно вашей текущей комнаты.</td></tr></table>";
        ...
        $t .= "</td><td valign=top><i>Постарайтесь выбрать наиболее подходящее место для склада, так как у вас не будет возможности его перенести в другое место.<br><br> Складом смогут пользоваться все члены Вашего клана.</i></td></tr></table></td></tr></table>";
        print "<script>window.top.domir('{$text}','{$t}');</script>";
    }
    else
    {
        print "<script>alert('Склад может принадлежать только клану.');</script>";
    }
}
?>
