...
if ($load == "unset") {
    ...
    while ($row_num) {
        ...
        $city_selection_possible=$row_num[15];
        $row_num=SQL_next_num();
    }
    if ($result) {
        SQL_free_result($result);
    ...
    if ($city_name <> "") {
        ...
        if ($action == 5) {
            ...
            if ($city_rank == 1) {
                ...
                $info .= "<tr bgcolor=F7FBFF><td  align=center><b>Банк</b></td><td  align=center>1</td><td  align=center>0 / 0</td></tr>";
                if ($city_dead == 0) {
                    $info .= "<tr bgcolor=F7FBFF><td  align=center><b>Усыпальница<br> с регенерацией</b></td><td  align=center>0</td><td  align=center>5000 / 0</td></tr>";
                } else {
                    $info .= "<tr bgcolor=F7FBFF><td  align=center><b>Усыпальница<br> с регенерацией</b></td><td  align=center>1</td><td  align=center>5000 / 0</td></tr>";
                }
                $info .= "<tr bgcolor=F7FBFF><td  align=center><b>Стража</b></td><td  align=center>$city_npc</td><td  align=center>1000 / 0</td></tr>";
            }
            $info .= "</table>";
        } else {
        ...
        if (($rank_opt1 == 1) || ($city_rank == 1)) {
            if ($action == 11) {
                ...
                if ($do == "add") {
                    ...
                    $nname = '';
                    $SQL='select id,name,clan,level from sw_users where (up_name="'.$up_name.'" and city <> 1) or (clan = '.$city_id.')';
                    $row_num=SQL_query_num($SQL);
                    $clan_members = 0;
                    while ($row_num) {
                        if ($name2 == $row_num[1]) {
                    ...
                    if (($city_type == 2 && $clan_members < 100) || $clan_members < 80) {	// city_type 2 - Орден
                        if (($nname <> "") && ($level>=10)) {
                            if ($clan == 0) {
                        ...
                        $info .= '<table cellpadding=4><tr><td><b>Вы уже достигли максимального количества игроков для своего типа клана.</b></td></tr></table>';
                    }
                }
                $r = rand(0, 999999);
                if ($city_rank == 1) {
...
{
    $telegram_chat_token = uniqid('tg_', true);
    $SQL = "telegram_chat_token from sw_users where telegram_chat_token=$telegram_chat_token";
    $row_num = SQL_query_num($SQL);
    if (!empty($row_num)) {
