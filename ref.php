...
{
	$level++;
	$level_up_db_update = ",level=$level,s_up=s_up+2";

	// Change money bonus depending on level
	if ($level < 40)
	{
		$level_up_db_update .= ",gold=gold+20";
	}
	else if ($level < 60)
	{
		$level_up_db_update .= ",gold=gold+30";
	}
	else if ($level < 80)
	{
		$level_up_db_update .= ",gold=gold+40";
	}
	else if ($level < 100)
	{
		$level_up_db_update .= ",gold=gold+60";
	}
	else if ($level < 120)
	{
		$level_up_db_update .= ",gold=gold+80";
	}
	else if ($level < 160)
	{
		$level_up_db_update .= ",gold=gold+140";
	}
	else
	{
		$level_up_db_update .= ",gold=gold+200";
	}

	if ($level % 20 == 0)
	{
		$level_up_db_update .= ",h_up=h_up+1";
	}
