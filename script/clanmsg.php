...
if ( $allow == 1 )
{
    if ( $action == "del" )
    {
        $SQL = "delete from sw_msg where id={$read_id} and to_id={$city_id} and city=0";
        sql_do( $SQL );
    }
    if ( $action == "new2" && $text != "" && $title != "" )
    {
        ...
        $SQL = "insert into sw_msg(from_id,from_name,to_id,to_name,city,text,title,date) values ({$city_id},'{$c_name[$city_id]}<br>{$player_name}',{$to_id},'{$c_name[$to_id]}',0,'{$text}','{$title}',NOW())";
        sql_do( $SQL );
    }
    if ( $action == "new" )
    {
        ...
        print "<br><table width=98% align=center bgcolor=8C9AAD cellpadding=3 cellspacing=1>\r\n\t\t\t<form action=clanmsg.php method=post><input type=hidden name=action value=new2>\r\n\t\t\t<tr bgcolor=C6CACF><td align=center><b>Новое сообщение</b></td></tr>\r\n\t\t\t<tr bgcolor=E6EAEF><td>\r\n\t\t\t<table width=100%><tr><td width=90><b>Получатель:</b></td><td>{$optend}</td></tr>\r\n\t\t\t<tr><td width=90><b>Тема (255):</b></td><td><input type=text name=title value='{$title}' size=35></td></tr>\r\n\t\t\t<tr><td width=90><b>Текст (2000):</b></td><td><textarea cols=35 rows=8 name=text></textarea></td></tr>\r\n\t\t\t</table>\r\n\t\t\t</td></tr>\r\n\t\t\t<tr bgcolor=C6CACF><td><table width=100%><tr><Td width=33%><a href=clanmsg.php?ran={$ran}&page={$page} class=menu><b>Назад</b></a></td><td align=center><input type=submit value=Отправить></td><td align=right width=33%><input type=Reset value=Сбросить></td></tr></table></td></tr>\r\n\t\t\t</form>\r\n\t\t\t</table>\r\n\t\t\t";
    }
    else if ( $action == "read" )
    {
        $SQL = "select id,from_id,from_name,to_id,to_name,isread,text,title,date from sw_msg where id={$read_id} and city=0";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            ++$i;
        ...
        if ( $read == 0 )
        {
            $SQL = "update sw_msg set isread=1 where id={$read_id} and city=0";
            sql_do( $SQL );
        }
        print "<br><table width=98% align=center bgcolor=8C9AAD cellpadding=3 cellspacing=1>\r\n\t\t\t<tr bgcolor=C6CACF><td align=center><b>Сообщение</b></td></tr>\r\n\t\t\t<tr bgcolor=E6EAEF><td>\r\n\t\t\t<table width=100%><tr><td width=60><b>Отправил:</b></td><td align=center width=80>{$from_name}</td><td>&nbsp;</td><td width=60 align=center><b>Дата:</b></td><td width=80 align=center>{$date}</td></tr>\r\n\t\t\t<tr><td colspan=5>\r\n\t\t\t<br><b>Тема: </b>{$title}<br>\r\n\t\t\t<br><b>Текст: </b>{$text}<br><br>\r\n\t\t\t</td></tr>\r\n\t\t\t</table>\r\n\t\t\t</td></tr>\r\n\t\t\t<tr bgcolor=C6CACF><td><table width=100%><tr><Td width=33%><a href=clanmsg.php?ran={$ran}&page={$page} class=menu><b>Назад</b></a></td><td align=center><a href='clanmsg.php?ran={$ran}&page={$page}&action=new&to_id={$from_id}&title=re:{$title}' class=menu><b>Ответить</b></a></td><td align=right width=33%><a href=clanmsg.php?ran={$ran}&page={$page}&action=del&read_id={$id} class=menu><b>Удалить</b></a></td></tr></table></td></tr>\r\n\t\t\t</table>\r\n\t\t\t";
    }
    else
    {
        $SQL = "select count(*) as num from sw_msg where to_id={$city_id} and city=0";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            $count = $row_num[0];
        ...
        for ( ; $i < $count; $i = $i + 15 )
        {
            ...
            {
                $p .= "|<a href=clanmsg.php?page={$i} class=menu>{$i}-{$e}</a>|";
            }
        }
        print "<br><div align=center>| <a href=clanmsg.php?ran={$ran}&page={$page}&action=new class=menu><b>Новое сообщение</b></a> |<br>{$p}</div>";
        ...
        $i = 0;
        $SQL = "select id,from_id,from_name,to_id,to_name,isread,text,title,date from sw_msg where to_id={$city_id} and city=0 order by date desc limit {$page},15";
        $row_num = sql_query_num( $SQL );
        while ( $row_num )
        {
            ++$i;
