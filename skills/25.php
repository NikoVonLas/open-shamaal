...
if ( $num == 1 )
{
    ...
    {
        $room_name = "Защитная или боевая зона";
        $tn = $target_name;
        $target_id = $player_id;
        $target_name = $player_name;
    }
...
if ( $num == 1 )
{
    if ( $tn != "" )
    {
        $t = "[<b>{$target_name}</b>]&nbsp;{$tn} находится  \xE2 локации : <b>{$room_name}</b>.";
    }
    else
    {
        $t = "[<b>{$target_name}</b>]&nbsp;{$tn} телепатического соединения не выбрана.";
    }
    $t = "window.top.add(\"{$time}\",\"\",\"{$t}\",5,\"\");";
    print "{$t}";
}
...
$game_skill_name[25][5] = "Телепортироваться";
...
$game_skill_afflict_text[25][5] = "[<b>{$old}</b>]&nbsp;<b>{$player_name} </b>телепортировал{$las} \xE2 Вашу комнату.";
