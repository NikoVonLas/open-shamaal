...
$SQL = "select id as num from sw_users where npc=0 and online>{$time}-604800";
$row_num = sql_query_num( $SQL );
$active_players = array();
while ( $row_num )
{
    array_push($active_players, $row_num[0]);
    $row_num = sql_next_num( );
}
if ( $result )
{
...
fputs( $file, $all );
fputs( $file, "\n" );
fputs( $file, count($active_players));
fputs( $file, ':');
fputs( $file, join(',', $active_players));
fputs( $file, "\n" );
