...
if ( $fid != "" && ( $owner_id == $player_id && $owner_city == 0 || $owner_id == $player_clan && $owner_city == 3 ) )
{
    ...
    if ( $what == "sunduk" )
    {
        if ( $do == "show" )
        {
            ...
            if ($show != '') {
                $num = getobjinfo("sw_obj.owner = {$player_room} and room = 1 and sw_stuff.specif not in ($specif_filter) and sw_stuff.obj_place={$show} order by sw_obj.price, sw_obj.id", "", "name=act value=buy");
            }
            else {
