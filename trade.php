...
function noTrade()
{
	...
	$text = "<b>Торговля остановлена, предлагаемые предметы, по какой-то причине, не найдены в рюкзаке у одного из участников сделки.</b>";
	$time = date("H:i");
			...
			if ($stg == 1)
			{
				...
				if ($action == "addobj")
				{
					...
					if ($c_num <=2)
					{
						...
						print "alert('В обмене может участвовать не более 3 объектов.');";
				}
			}
			else
			{
